...
export default function PropertiesPage() {
  ...
  const [isViewPropertyDetailsModalOpen, setIsViewPropertyDetailsModalOpen] =
    useState(false);
  ...
  const [showDirectDeleteConfirmation, setShowDirectDeleteConfirmation] =
    useState(false); // True for direct delete, false for admin request
  ...
  const handleSubmitProperty = (e) => {
    ...
    if (currentProperty) {
      ...
      if (oldProperty.name !== newPropertyData.name) {
        changedFields.push(
          `name from "${oldProperty.name}" to "${newPropertyData.name}"`
        );
      }
      if (oldProperty.address !== newPropertyData.address) {
        changedFields.push(
          `address from "${oldProperty.address}" to "${newPropertyData.address}"`
        );
      }
      ...
      if (oldProperty.rentPerRoom !== newPropertyData.rentPerRoom) {
        rentChanged = true;
        changedFields.push(
          `rent per room from ₦${oldProperty.rentPerRoom.toLocaleString()} to ₦${newPropertyData.rentPerRoom.toLocaleString()}`
        );
        ...
        tenantsToUpdate = tenantsToUpdate.map((tenant) => {
          if (tenant.propertyId === currentProperty.id) {
            ...
            if (
              ...
              tenant.paymentStatus === "part_payment"
            ) {
  ...
  const confirmDirectDelete = () => {
    if (propertyToDelete) {
      ...
      showMessage(
        `Property "${propertyToDelete.name}" has been permanently deleted.`,
        "success"
      );
  ...
  const requestAdminDeletion = () => {
    if (propertyToDelete) {
      ...
      showMessage(
        `Deletion request for "${propertyToDelete.name}" sent to admin for review.`,
        "info"
      );
      setPropertyToDelete(null);
      setIsDeletePropertyModalOpen(false);
  ...
  const handleSubmitMaintenanceRequest = (e) => {
    ...
    const tenantsInProperty = tenants.filter(
      (t) => t.propertyId === propertyForMaintenance.id
    );
      ...
      tenantsInProperty.forEach((tenant) => {
        ...
        addNotification(
          ...
          "general", // Using 'general' for now, could be a new type
          {
            tenantId: tenant.id,
            propertyName: propertyForMaintenance.name,
            date: maintenancePreferredDate,
            time: maintenancePreferredTime,
          }
        );
      });
    } else {
      showMessage(
        `Maintenance request submitted for "${propertyForMaintenance.name}".`,
        "info"
      );
    }
    ...
    <div className="properties-page-container">
        ...
        {properties.length > 0 ? (
          properties.map((property) => (
            <div key={property.id} className="property-card">
              ...
              <p className="property-detail">
                Occupied Rooms: {getOccupiedRooms(property.id)}
              </p>
              <p className="property-detail">
                Rent per Room: ₦{property.rentPerRoom.toLocaleString()}
              </p>
              <p className="property-detail">
                Lease Period: {property.leasePeriod || "N/A"} months
              </p>{" "}
              ...
              <div className="bottom">
                ...
                <button
                  className="button-secondary"
                  onClick={() => handleRequestMaintenance(property)}
                >
                  Request Maintenance
                </button>
                <button
      ...
      >
        <form onSubmit={handleSubmitProperty} className="form-spacing">
          ...
          <div className="form-group">
            <label htmlFor="totalRooms" className="form-label">
              Total Rooms
            </label>
            <div>
              <input
          ...
          <div className="form-group">
            <label htmlFor="rentPerRoom" className="form-label">
              Rent per Room (₦)
            </label>
            <div>
              <input
          ...
          <div className="form-group">
            <label htmlFor="propertyImageUrl" className="form-label">
      ...
      <Modal
        isOpen={isViewPropertyDetailsModalOpen}
        onClose={() => setIsViewPropertyDetailsModalOpen(false)}
        ...
        {selectedProperty ? (
          <div className="property-detail-content">
              ...
              <div className="property-image-container-modal">
                <img
                  ...
                  onError={(e) => {
                    e.target.onerror = null;
                    e.target.src =
                      "https://placehold.co/600x300/E0E0E0/000000?text=No+Image";
                  }}
                />
              </div>
            )}
            ...
            <p>
              ...
              {getOccupiedRooms(selectedProperty.id)}
            </p>
            <p>
            ...
            <p>
              <strong>Rent per Room:</strong> ₦
              {selectedProperty.rentPerRoom.toLocaleString()}
            </p>
            <p>
              <strong>Default Lease Period:</strong>{" "}
              {selectedProperty.leasePeriod || "N/A"} months
            </p>
            <h4 className="modal-subtitle">Tenants in this Property:</h4>
            {tenants.filter((t) => t.propertyId === selectedProperty.id)
              .length > 0 ? (
            ...
            {showDirectDeleteConfirmation ? (
              <>
                ...
                <p className="mb-6 text-gray-700">
                  This property has no tenants and will be permanently removed.
              ...
              <>
                ...
                <p className="mb-6 text-gray-700">
                  ...
                  Deletion will not occur until approved by the admin.
                </p>
                <div className="bottom">
                  <button
                    type="button"
      ...
      <Modal
        isOpen={isMaintenanceRequestModalOpen}
        onClose={() => setIsMaintenanceRequestModalOpen(false)}
        title={`Request Maintenance for ${propertyForMaintenance?.name || ""}`}
      >
        {propertyForMaintenance && (
          ...
          >
            ...
            <div className="form-group">
              <label htmlFor="maintenanceUrgency" className="form-label">
                Urgency
              </label>
              <select
            ...
            <div className="form-group">
              <label htmlFor="maintenancePreferredDate" className="form-label">
                Preferred Date
              </label>
              <div>
                <input
            ...
            <div className="form-group">
              <label htmlFor="maintenancePreferredTime" className="form-label">
            ...
            <div className="bottom">
              <button
                type="button"
                className="button-secondary"
                onClick={() => setIsMaintenanceRequestModalOpen(false)}
              >
                Cancel
              </button>
              <button type="submit" className="button-primary">
